...
/-- Output of one FNO tape construction. -/
...
The FNO wrapper rebuilds its tape for every sample. Explicit disposal is therefore part of
...
def dispose (fw : Forward) : IO Unit := do
  ...
  if released > fw.ownedBufferCount then
    throw <| IO.userError "autograd: fno1d-rfft: invalid forward buffer release count"

end Forward

/-- Minimal Adam state carried across FNO training steps. -/
...
/--
Initialize parameters for the FNO1D model:
...
/-- Fetch a parameter with an error message that points to the FNO wrapper. -/
def getParam (ps : Array Param) (i : Nat) : Result Param :=
  match ps[i]? with
  | some p => pure p
  | none => throw s!"autograd: fno1d-rfft: parameter index out of bounds: {i}"
...
def addParamLeaf (t : Tape) (ps : Array Param) (paramBuffers : Array Buffer)
  ...
  let buf ← match paramBuffers[i]? with
    | some buf => pure buf
    | none => throw s!"autograd: fno1d-rfft: missing uploaded parameter buffer: {i}"
  let checked ← match AnyBuffer.validate { s := p.shape, buf := buf } with
    | .ok checked => pure checked
    | .error msg => throw s!"autograd: fno1d-rfft: invalid uploaded parameter {i}: {msg}"
  let (t', id) := t.leaf
    checked
    (some s!"param{i}")
...
/--
Build a CUDA tape that computes prediction (and optionally MSE loss) for the real-RFFT FNO.
...
def forwardWithBuffers (grid width modes blocks : Nat)
  ...
  let checkedInput ← match AnyBuffer.validate { s := xMatShape, buf := xBuffer } with
    | .ok checked => pure checked
    | .error msg => throw s!"autograd: fno1d-rfft: invalid uploaded input: {msg}"
  ...
  | some _ =>
      let targetBuffer ← match targetBuffer? with
        | some buf => pure buf
        | none => throw "autograd: fno1d-rfft: missing uploaded target buffer"
...
/--
Build one FNO tape from fresh CUDA uploads.
...
def predFromTape (grid : Nat) (t : Tape) (id : Nat) : IO (Result (Tensor Float ([grid]))) := do
  ...
  | .ok b =>
      ...
      | some y => pure <| .ok y
      | none => pure <| .error "autograd: fno1d-rfft: prediction shape mismatch"
...
def meanLoss (grid width modes blocks : Nat) (ps : Array Param)
  ...
  if samples.size == 0 then
    return .error "autograd: fno1d-rfft: empty evaluation stream"
  let compute : ExceptT String IO (Tensor Float [samples.size]) :=
    Tensor.generateFlatM [samples.size] fun index => do
      ...
      ExceptT.mk <| try
        ...
        | none => pure (.error "autograd: fno1d-rfft: internal missing loss id")
      finally fw.dispose
  return (← compute.run).map Tensor.mean

/-- Tensor Adam update with cached bias corrections and explicit scalar evaluation order.

Products and quotients are evaluated as `(1-beta2)*g*g` and
`lr*(m/biasCorr1)/(sqrt(v/biasCorr2)+eps)`. Tensor shapes enforce equal parameter and
gradient sizes.
-/
...
/--
Run reverse-mode on the FNO tape and update every recorded parameter with Adam.

Gradients are computed on CUDA buffers and downloaded to host tensors before the Adam update.
-/
def prepareAdamBackward
  ...
  let lossId ← match fw.lossId? with
    | some id => pure id
    | none => return .error "autograd: fno1d-rfft: internal missing loss id"
  ...
  let grads ← try
      ...
      return .error s!"autograd: fno1d-rfft: sparse backward failed: {e}"
      ...
      for i in [:ps.size] do
        ...
        let nodeId ← match fw.paramIds[i]? with
          ...
          | none =>
              ...
              fw.dispose
              return .error "autograd: fno1d-rfft: internal missing param id"
        let gAny ← match grads.get? nodeId with
          | some g => pure g
          | none =>
              Tape.releaseSparseGrads grads
              fw.dispose
              return .error "autograd: fno1d-rfft: internal missing grad"
        if _h : gAny.s = p.shape then
          ...
          let gradient ← match Convert.unflattenFloat? (s := p.shape) grad with
            ...
            | none =>
                Tape.releaseSparseGrads grads
                fw.dispose
                return .error "autograd: fno1d-rfft: gradient buffer length mismatch"
          ...
          if hi : i < out.size then
            ...
            Tape.releaseSparseGrads grads
            fw.dispose
            return .error "autograd: fno1d-rfft: internal update index invalid"
        else
          Tape.releaseSparseGrads grads
          fw.dispose
          return .error "autograd: fno1d-rfft: gradient shape mismatch"
      Tape.releaseSparseGrads grads
      fw.dispose
      pure <| .ok (out, st')

/-- Predict through the real-RFFT spectral path and release the temporary tape after downloading. -/
...
/-- Consume one FNO training tape, returning updated parameters and cached Adam state. -/
