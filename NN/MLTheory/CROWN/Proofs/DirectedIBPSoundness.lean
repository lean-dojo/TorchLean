...
private theorem rowEncloses_iff {d : Nat} {lo hi : Tensor α [d]} {f : Nat → ℝ} :
...
private theorem unary_step {dims : Nat → Nat} {v : Nat → Nat → ℝ} {id p : Nat} {F : ℝ → ℝ}
...
private theorem unary_lookup {parents : Array Nat} {boxes : Array (Option (FlatBox α))}
...
omit [BoundOps α] [LawfulBoundOps α] in
private theorem binary_lookup {parents : Array Nat} {boxes : Array (Option (FlatBox α))}
